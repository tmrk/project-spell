const messages = Object.freeze({
  appControls: 'Game controls',
  hearAgain: 'Hear the word again',
  turnMusicOff: 'Turn music off',
  turnMusicOn: 'Turn music on',
  openSettings: 'Open parent settings',
  projectName: 'Project Spell',
  language: 'Language',
  play: 'Play',
  home: 'Back to the start screen',
  greetingFirstSpeeches: Object.freeze([
    'Hi {name}!',
    'Hello {name}!',
    'Welcome, {name}!',
    'Yay, {name}!',
  ]),
  greetingReturningSpeeches: Object.freeze([
    'Welcome back, {name}!',
    'Hi again, {name}!',
    'Good to see you, {name}!',
    'Yay, {name}’s back!',
  ]),
  spellPrompt: 'Spell the word {word}',
  roundFinishedSpeeches: Object.freeze([
    'Amazing! You finished the round!',
    'Awesome work! You finished the round!',
    'Great spelling! You finished the round!',
    'Fantastic! You finished the round!',
  ]),
  superRoundHeading: 'Super round!',
  superRoundIntroSpeeches: Object.freeze([
    'Super round! Ready?',
    'It’s a super round! Let’s go!',
  ]),
  superRoundCountdownOne: 'One more round to the super round!',
  superRoundCountdownMany: '{count} rounds to the super round',
  superRoundDone: 'Super round finished — what a star!',
  newShinyStickerAria: 'A shiny sticker for your book!',
  tryAgain: 'Try one more time',
  wordFinished: 'That’s the word!',
  wordFinishedSpeeches: Object.freeze([
    'Great!',
    'Well done!',
    'Awesome!',
    'Excellent!',
    'Nice work!',
    'Fantastic!',
  ]),
  encouragementSpeeches: Object.freeze([
    'Almost — try again!',
    'You can do it!',
    'So close!',
    'Give it another try!',
  ]),
  correctMessages: Object.freeze([
    'Nice!',
    'Yes!',
    'Awesome!',
    'You got it!',
    'Great job!',
    'Exactly!',
    'Well done!',
    'Wonderful!',
  ]),
  progress: 'Word {current} of {total}',
  progressCount: '{current} / {total}',
  letterWord: '{count} letter word',
  typeNextLetter: 'Type the next letter',
  completeHeading: 'Awesome spelling!',
  roundStarsEarned: '{count} stars for this round',
  starJarLine: '★ {count} stars in your jar',
  stickerBookHeading: 'My sticker book',
  stickerBookCount: '{count} stickers',
  stickerBookPageCount: '{theme} · {count} stickers',
  themeAnimals: 'Animals',
  themeFood: 'Food',
  themeThings: 'Things',
  themeShiny: 'Shiny',
  nextPage: 'Next page',
  previousPage: 'Previous page',
  openStickerBook: 'Open sticker book',
  closeStickerBook: 'Close sticker book',
  newStickerAria: 'New sticker: {word}',
  medalCaseHeading: 'Medals',
  badgeFirstRound: 'First round',
  badgeWords10: '10 words',
  badgeWords50: '50 words',
  badgeWords100: '100 words',
  badgePerfectRound: 'Perfect round',
  badgeNormalRound: 'Spelling star',
  nextRound: 'Next round',
  startSuperRound: 'Super round',
  resumeRound: 'Keep going',
  startFresh: 'Start a new game',
  whoIsPlayingHeading: 'Who is playing?',
  letterCompleted: 'completed',
  letterCurrent: 'current letter',
  letterNext: 'next',
  letterLabel: '{letter}, {state}',
  letterHiddenLabel: 'hidden letter, {state}',
  modeHeading: 'Game mode',
  modeEasyLabel: 'Easy',
  modeEasyDescription: 'The letters are shown — find them on the keyboard',
  modeNormalLabel: 'Normal',
  modeNormalDescription: 'The letters are hidden — spell the word you hear',
  modeCardEasy: 'I can see',
  modeCardNormal: 'I listen',
  playEasyAria: 'Play with the letters shown',
  playNormalAria: 'Play by listening to the word',
  nameReady: 'That’s me!',
  modeNormalNeedsSpeech: 'Normal mode works best with “Say each word” switched on.',
  settingsHeading: 'Settings',
  settingsHeadingSuffix: '(for parents)',
  savedToast: '✓ Saved',
  groupGame: 'Game',
  groupWords: 'Words',
  groupSound: 'Sound & look',
  groupProgress: 'Progress & about',
  groupProfiles: 'Who is playing',
  addProfile: 'Add a name',
  nameEntryTitle: 'What is your name?',
  renameProfileTitle: 'Change name',
  nameLabel: 'First name',
  namePlaceholder: 'Type a name',
  saveName: 'Save',
  cancelName: 'Cancel',
  renameProfile: 'Rename',
  deleteProfile: 'Delete',
  deleteProfileTitle: 'Delete {name}?',
  deleteProfileWarning: 'This erases {name}’s stars, stickers and settings. It cannot be undone.',
  deleteProfileConfirm: 'Delete',
  switchProfile: 'Play as {name}',
  profileDataNote: 'This is {name}’s play data.',
  closeSettings: 'Close settings',
  quickStart: 'A quick starting point',
  presetStarterLabel: 'Starter',
  presetStarterDescription: 'Short, one-beat words',
  presetExplorerLabel: 'Explorer',
  presetExplorerDescription: 'A friendly mix',
  presetChallengeLabel: 'Challenge',
  presetChallengeDescription: 'Longer words',
  round: 'Round',
  shortestWord: 'Shortest word',
  longestWord: 'Longest word',
  lettersOption: '{count} letters',
  syllables: 'Syllables',
  anyNumber: 'Any number',
  one: 'One',
  two: 'Two',
  three: 'Three',
  fourOrMore: 'Four or more',
  wordsInARow: 'Words in a row',
  wordsOption: '{count} words',
  yourWords: 'Your words',
  wordList: 'Word list',
  customWordsPlaceholder: 'grandma\nSaturday\nladybug',
  customWordsHelp: 'One word per line or separated by commas. Use 2–14 English letters.',
  chooseFrom: 'Choose from',
  allWords: 'Built-in words + my words',
  customWordsOnly: 'Only my words',
  letters: 'Letters',
  background: 'Background',
  paletteSunshine: 'Sunshine',
  palettePeach: 'Peach',
  paletteMint: 'Mint',
  cartoonEyes: 'Cartoon eyes',
  onScreenKeyboard: 'Letter keys on screen',
  keyboardSystem: 'No letter keys',
  keyboardFull: 'All letters',
  keyboardSimple: 'A few letters',
  keyboardHelp: 'Big capital letter keys under the word. The device keyboard keeps working either way.',
  keyboardLabel: 'Letter keys',
  acceptUnaccented: 'Accept unaccented typing',
  acceptUnaccentedHelp: 'For example, let a match å, ä, or á.',
  autoLadder: 'Start short, grow longer as words are mastered',
  autoLadderHelp: 'Project Spell begins with shorter words and gently unlocks longer ones.',
  adaptivePractice: 'Practice tricky words a little more often',
  adaptivePracticeHelp: 'Words your child finds hard come back sooner. They will not notice.',
  sound: 'Sound',
  sayEachWord: 'Say each word',
  spellBack: 'Spell the word back after it is finished',
  spellBackHelp: 'Each letter is named again, then the whole word.',
  soundEffects: 'Sound effects',
  backgroundMusic: 'Background music',
  wordsMatch: '{count} {unit} match',
  wordSingular: 'word',
  wordPlural: 'words',
  packAll: 'All words',
  packAnimals: 'Animals',
  packFood: 'Food',
  packVehicles: 'Vehicles',
  packNature: 'Nature',
  noWordsMatch: 'No words match these settings',
  allWordsMastered: 'Every matching word is mastered — brilliant! Add more words or raise the filters.',
  matchingFilters: 'Matching your filters',
  matchPill: '✓ {count} {unit}',
  resetAll: 'Reset all settings',
  languageChangeTitle: 'Change language?',
  languageChangeWarning: 'This will restart the game and return to the start page.',
  keepLanguage: 'Keep current language',
  changeAndRestart: 'Change & restart',
  progressHeading: 'Progress',
  progressWordsPractised: '{count} words practiced',
  progressRoundsFinished: '{count} rounds finished',
  progressPlayTime: '{minutes} minutes of play',
  progressTrickyLetters: 'Tricky letters: {letters}',
  progressNoData: 'No play data yet — statistics appear after the first words are spelled.',
  masteredCountLine: '{count} words mastered',
  letterHeatMap: 'Letter by letter',
  letterHeatMapHelp: 'Cooler letters are going well. Warmer ones are worth a little practice.',
  letterAccuracy: '{letter}: {percent}% right, {count} tries',
  confusionsHeading: 'Mixed up most often',
  confusionPair: '{expected} typed as {typed}',
  downloadData: 'Download data',
  clearProgress: 'Clear progress',
  clearProgressTitle: 'Clear progress?',
  clearProgressWarning: 'This erases all locally stored play statistics and rewards. It cannot be undone.',
  clearProgressConfirm: 'Clear everything',
  clearProgressCancel: 'Keep my data',
  aboutHeading: 'About',
  aboutCredits: 'Credits and licenses',
});

export default Object.freeze({
  code: 'en-US',
  flag: '🇺🇸',
  label: 'US English',
  voiceNamePattern: /samantha|ava|allison|susan|zira|google us english/iu,
  messages,
});
